...
<head>
    ...
    <title>Gestionar Negocios - Panel Admin</title>
...
<body>
    <div class="admin-container">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1.5rem;">
            <h1 class="page-title" style="margin: 0;">Gestionar Negocios</h1>
        ...
        <table>
            ...
            <tbody>
                @forelse($businesses ?? [] as $business)
                    <tr>
                        <td>{{ $business->id }}</td>
                        <td><strong>{{ $business->name ?? 'Sin nombre' }}</strong></td>
                        <td>{{ isset($business->category) && $business->category ? $business->category->name : 'Sin categoría' }}</td>
                        <td>{{ $business->email ?? 'N/A' }}</td>
                        <td><small>{{ $business->phone ?? 'N/A' }}</small></td>
                        <td>
                            @if(isset($business->user) && $business->user)
                                {{ $business->user->email ?? 'N/A' }}
                            @else
                                <span style="color: #6c757d;">Sin usuario</span>
                            @endif
                        </td>
                        <td>
                        ...
                        <td>
                            @if($business->is_active ?? false)
                                <form action="{{ route('admin.businesses.toggle', $business->id) }}" method="POST" style="display: inline;">
                                    @csrf
                            ...
                            @endif
                            <form action="{{ route('admin.businesses.delete', $business->id) }}" method="POST" style="display: inline;" onsubmit="return confirm('¿Estás seguro de eliminar este negocio?');">
                                @csrf
                                @method('DELETE')
        ...
        @if(isset($businesses) && method_exists($businesses, 'links'))
            <div class="pagination-info">
                Mostrando {{ $businesses->firstItem() ?? 0 }} a {{ $businesses->lastItem() ?? 0 }} de {{ $businesses->total() }} resultados
            </div>
