        ...
        <main class="main-content">
            ...
            <div id="page-dashboard" class="page-content active">
                ...
                <div class="stats-grid">
                    <div class="stat-card">
                        ...
                        <div class="stat-info">
                            ...
                            <p class="stat-number">{{ $stats['active_promotions'] ?? 0 }}</p>
                        </div>
                    </div>
                    <div class="stat-card">
                    ...
                    <div class="stat-card">
                        ...
                        <div class="stat-info">
                            <h3>Clics</h3>
                            <p class="stat-number">{{ number_format($stats['total_clicks'] ?? 0) }}</p>
                        </div>
                    </div>
                    <div class="stat-card">
                        ...
                        <div class="stat-info">
                            <h3>Ingresos</h3>
                            <p class="stat-number">${{ number_format($stats['total_revenue'] ?? 0, 0, ',', '.') }}</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="dashboard-section">
                    ...
                    @if(isset($recentPromotions) && $recentPromotions->count() > 0)
                            ...
                            @foreach($recentPromotions as $promo)
                                <div class="promo-item">
                                    ...
                                    <div class="promo-details">
                                        <h4>{{ $promo->title ?? 'Sin título' }}</h4>
                                        <p>Activa hasta: {{ isset($promo->end_date) && $promo->end_date ? $promo->end_date->format('d/m/Y') : 'N/A' }}</p>
                                    </div>
                                    <div class="promo-status {{ (isset($promo) && method_exists($promo, 'isActive') && $promo->isActive()) ? 'active' : '' }}">
                                        {{ (isset($promo) && method_exists($promo, 'isActive') && $promo->isActive()) ? 'Activa' : 'Inactiva' }}
                                    </div>
                                </div>
                            @endforeach
                        </div>
            ...
            <div id="page-new-promotion" class="page-content">
                ...
                <div class="promotion-form-container">
                    <form id="newPromotionForm" class="promotion-form" action="{{ route('promotions.store') }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        <div class="form-group">
                            <label for="promo-title">Título de la Promoción *</label>
                            <input type="text" id="promo-title" name="title" value="{{ old('title') }}" placeholder="Ej: 2x1 en Hamburguesas" required>
                            @error('title')
                        ...
                        <div class="form-group">
                            ...
                            <textarea id="promo-description" name="description" rows="5" placeholder="Describe tu promoción de manera atractiva..." required>{{ old('description') }}</textarea>
                            @error('description')
                                <span class="error-message">{{ $message }}</span>
                            @enderror
                        </div>
                        <div class="form-row">
                            <div class="form-group">
                                ...
                                <input type="number" id="promo-price-regular" name="price_regular" value="{{ old('price_regular') }}" placeholder="25000" step="0.01" min="0" required>
                                @error('price_regular')
                                    <span class="error-message">{{ $message }}</span>
                                @enderror
                            </div>
                            <div class="form-group">
                                ...
                                <input type="number" id="promo-price-discount" name="price_discount" value="{{ old('price_discount') }}" placeholder="17500" step="0.01" min="0" required>
                        ...
                        <div class="form-row">
                            <div class="form-group">
                                ...
                                <input type="date" id="promo-start-date" name="start_date" value="{{ old('start_date') }}" required>
                                @error('start_date')
                                    <span class="error-message">{{ $message }}</span>
                                @enderror
                            </div>
                            <div class="form-group">
                                ...
                                <input type="date" id="promo-end-date" name="end_date" value="{{ old('end_date') }}" required>
                                @error('end_date')
                                    <span class="error-message">{{ $message }}</span>
                                @enderror
                            </div>
                        ...
                        @foreach($recentPromotions as $promo)
                            <div class="promotion-list-item">
                                ...
                                <div class="promo-list-content">
                                    ...
                                    <p class="promo-list-desc">{{ \Illuminate\Support\Str::limit($promo->description ?? 'Sin descripción', 100) }}</p>
                                    <div class="promo-list-meta">
                                        <span><i class="fas fa-calendar"></i> {{ $promo->start_date ? $promo->start_date->format('d/m/Y') : 'N/A' }} - {{ $promo->end_date ? $promo->end_date->format('d/m/Y') : 'N/A' }}</span>
                                        <span><i class="fas fa-tag"></i> {{ $promo->category->name ?? 'Sin categoría' }}</span>
                                        <span><i class="fas fa-eye"></i> {{ number_format($promo->views_count ?? 0, 0, ',', '.') }} visualizaciones</span>
                                    </div>
                                    <div class="promo-list-price" style="margin-top: 0.5rem;">
                                        <span class="price-old" style="text-decoration: line-through; color: #999; margin-right: 0.5rem;">${{ number_format($promo->price_regular ?? 0, 0, ',', '.') }}</span>
                                        <span class="price-new" style="color: #E63946; font-weight: 700; font-size: 1.1rem;">${{ number_format($promo->price_discount ?? 0, 0, ',', '.') }}</span>
                                        <span class="discount-badge" style="background: #28a745; color: white; padding: 0.2rem 0.5rem; border-radius: 4px; font-size: 0.8rem; margin-left: 0.5rem;">-{{ $promo->discount_percentage ?? 0 }}%</span>
                                    </div>
                                </div>
                                <div class="promo-list-actions">
                                    <span class="status-badge {{ ($promo->is_active ?? false) ? 'active' : 'inactive' }}" style="padding: 0.4rem 0.8rem; border-radius: 4px; font-size: 0.85rem; font-weight: 600; {{ ($promo->is_active ?? false) ? 'background: #d4edda; color: #155724;' : 'background: #f8d7da; color: #721c24;' }}">
                                        {{ ($promo->is_active ?? false) ? 'Activa' : 'Inactiva' }}
                                    </span>
                                </div>
                            </div>
                        @endforeach
